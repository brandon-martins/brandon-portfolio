...
function matchFilter(cert, f) {
	...
	if (f === 'Standard Bank / Oracle')
		return (
			cert.issuer.includes('Standard Bank') || cert.issuer.includes('Oracle')
		);
			...
			<div className="max-w-6xl mx-auto px-4 sm:px-8">
				...
				<FadeIn delay={0.1} className="grid-auto-260 gap-4 mb-16">
					{education.map((edu, i) => (
						<div
							key={i}
							className="bg-accent-dim border border-accent rounded-sm p-5 md:p-6"
						>
							<div className="font-mono uppercase text-accent text-[0.68rem] tracking-[0.12em] mb-2">
								{edu.year}
							</div>
							<div className="font-display font-bold text-text text-[1.05rem] md:text-[1.1rem] leading-snug mb-1">
								{edu.degree}
							</div>
							{edu.grade && (
								<div className="font-mono text-accent-bright text-[0.75rem] mb-1">
									Grade: {edu.grade}
								</div>
							)}
				...
				<FadeIn delay={0.2} className="cert-grid">
					{filtered.map((cert, i) => (
						<div key={i} className="cert-row">
							<div
								className={`cert-icon ${isAWS(cert) ? 'cert-icon-aws' : 'cert-icon-default'}`}
							>
								{isAWS(cert) ? (
							...
							<div>
								<div className="font-medium text-text text-[0.85rem] leading-snug mb-1">
									{cert.name}
								</div>
								<div className="font-mono text-muted text-[0.68rem] tracking-[0.04em]">
