			...
			<div className="max-w-6xl mx-auto px-4 sm:px-8">
				<FadeIn className="mb-14">
					...
					<p className="text-muted mt-3 max-w-130">
						...
						engagements across Gauteng and South Africa.
					</p>
				</FadeIn>
					...
					<FadeIn delay={0.1}>
								...
								>
									...
									<div>
										...
										{href ? (
											<a
												href={href}
												className="text-text text-[0.92rem] font-medium no-underline hover:text-accent transition-colors duration-200"
											>
												{value}
											</a>
										) : (
											<span className="text-text text-[0.92rem] font-medium">
												{value}
											</span>
										)}
									</div>
						...
						<div className="badge-open">
							<div className="flex items-center gap-2 mb-2">
								...
								<span className="font-mono uppercase text-green text-[0.72rem] tracking-[0.08em]">
									Open to Opportunities
								</span>
							</div>
							<p className="text-muted text-[0.85rem] leading-[1.6]">
								Available for freelance projects, contract work, and full-time
								positions. Based in Johannesburg with remote capacity.
							</p>
						</div>
					</FadeIn>
					...
					<FadeIn delay={0.2}>
						<form onSubmit={handleSubmit} className="flex flex-col gap-4">
							<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
								{formFields.map((f) => (
									<div key={f.name}>
										<label className="block font-mono uppercase text-muted text-[0.68rem] tracking-widest mb-1.5">
											{f.label}
										</label>
										<input
											type={f.type}
